...
const AppSubmenu = (props) => {
    ...
    const renderLink = (item, i) => {
        ...
        if (item.to) {
            ...
            return (
                <Link tabIndex="0" aria-label={item.label} onKeyDown={onKeyDown} role="menuitem" href={item.url} className="p-ripple" onClick={(e) => onMenuItemClick(e, item, i)} target={item.target}>
                    {content}
                </Link>
            );
        }
